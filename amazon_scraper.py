...
class AmazonLaptopScraper:
    ...
    def __init__(self):
        # Stable browser-like header profiles per session
        ...
        self.session.verify = True  # TLS verification enabled

        # Optional proxy support (e.g. HTTP(S)_PROXY env)
    ...
    def _refresh_headers(self, referer=None):
        h = self._make_headers(referer=referer)
        self.session.headers.clear()
        self.session.headers.update(h)

    def _is_blocked_response(self, r):
        if r is None:
            return True
    ...
    def _build_search_url(self, search_term, page_num):
    ...
    def initialize_session(self):
        print("Session baslatiliyor ve cookies aliniyor...")
        for _ in range(3):
            try:
                self._refresh_headers()
                r = self.session.get(self.base_url, timeout=20)
                if not self._is_blocked_response(r):
                    print(f"[OK] Ana sayfa ziyaret edildi. Cookies: {list(self.session.cookies.keys())}")
                    time.sleep(random.uniform(1.5, 3.0))
                    return True
                status = r.status_code if r is not None else "None"
                print(f"[WARN] Ana sayfa bloklu: {status}")
            except Exception as e:
                print(f"[WARN] Session baslatma hatasi: {e}")
    ...
    def _extract_next_page_url(self, soup):
        if not soup:
            return None
        ...
        for i in range(retries):
            try:
                ...
                if r.status_code in (200, 203):
                    ...
                    # sometimes bot redirects happen
    ...
    def scrape_search_page_http(self, page_num=1, search_term="laptop"):
        ...
        if page_num > 1 and self.next_page_url:
            page_url = self.next_page_url
        else:
            page_url = self._build_search_url(search_term, page_num)

        referer = self.last_search_url
        print(f"\n[HTTP] Sayfa {page_num} istek: {page_url}")
        try:
            r = self._http_fetch(page_url, referer=referer)
            if self._is_blocked_response(r):
                status = r.status_code if r is not None else "None"
                print(f"  -> HTTP blocked (status {status}). Rotating profile and retrying...")
                self._rotate_profile()
                self._warmup_session()
                r = self._http_fetch(page_url, retries=2, backoff=3.0, referer=referer)
            ...
            if self._is_blocked_response(r):
                status = r.status_code if r is not None else "None"
                print(f"  - HTTP blocked or captcha detected (status {status})")
                return None, "captcha"
            ...
            print(f"  {len(cards)} urun bulundu (HTTP)")
            data = []
            for idx, card in enumerate(cards, 1):
                ...
                if idx == 1:
                    print(f"    [INFO] Ilk urun: {name[:60]}...")

            self.next_page_url = self._extract_next_page_url(soup)
            self.last_search_url = page_url
    ...
    def scrape_multiple_pages(self, max_pages=10, search_term="laptop", max_products=None):
        ...
        for page in range(1, max_pages + 1):
            ...
            force_retry = page <= 7
            if self.consecutive_failures >= 2 and not force_retry:
            ...
            while True:
                ...
                if ok:
                    successful_pages += 1
                    print(f"[OK] Sayfa {page} tarandi")
                    break

                attempt += 1
                if force_retry:
                    retry_wait = random.uniform(2.5, 6.0) + min(attempt, 6) * 0.6
                    print(f"  -> Sayfa {page} yanit vermedi, tekrar denenecek (deneme {attempt})")
                    print(f"  {retry_wait:.1f} sn bekleniyor...")
                    time.sleep(retry_wait)
                    continue

                print(f"[FAIL] Sayfa {page} taranamadi")
                break
